...
社内規定や議事録、顧客データのような機密文書を外部APIに一切送らず、閉域網だけで検索と回答を完結させるRAGエージェントです。このリポジトリには実装コード一式と、自分が主担当したRAGアーキテクチャの設計判断の記録が入っています。先に設計の話をして、そのあとに動かし方と評価の話をします。
...
全4フェーズの実装は終わっていますが、まだ完成とは言えません。GPU実機での50問実走による性能判定が最大の残作業で、その結果を待ってSparse検索の方式と埋め込みモデル(e5-large vs ruri-v3)の切り替えを決める予定です。コンテナのread_only化が実運用で悪さをしないかも、初回のGPU起動で見る必要があります。
